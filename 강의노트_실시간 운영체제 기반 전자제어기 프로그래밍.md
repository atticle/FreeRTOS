...
**시리얼 모니터**
Putty와 같은 외부 프로그램보다는 VSCode에서 `Serial Monitor` 확장을 검색한 후 설치해 사용한다.
...
| 시간 | 내용 | 산출물 |
| --- | --- | --- |
| 0:00 - 0:40 | RTOS가 필요한 이유와 장비 확인 | 보드 및 시리얼 터미널 연결 |
| 0:40 - 2:00 | CubeMX 기준 프로젝트 구성 | `rtos_day1` 프로젝트 생성 |
| 2:00 - 3:20 | 첫 태스크와 `osDelay` | `LED2` 주기 점멸 |
| 3:20 - 4:20 | 두 태스크와 스케줄러 | `LED1`, `LED2`의 독립 점멸 |
| 4:20 - 5:30 | `osDelay`와 `osDelayUntil` | 10ms 기준 시각 실험 |
| 5:30 - 6:40 | 우선순위와 기아 상태 | 바쁜 반복문 관찰과 복구 |
| 6:40 - 8:00 | UART 관찰, 정리, 재현 시험 | 틱 기록표와 확인 결과 |
...
| 시간 | 내용 | 산출물 |
| --- | --- | --- |
| 0:00 - 0:50 | 데이터와 사건의 차이 | 객체 선택표 |
| 0:50 - 2:30 | 큐로 버튼 처리와 LED 제어 분리 | StartButtonPollTask와 StartLedOutputTask |
| 2:30 - 4:20 | EXTI와 이진 세마포어 | SW1 사건 처리 태스크 |
| 4:20 - 5:30 | 디바운싱과 계수 세마포어 | 사건 유실 관찰 |
| 5:30 - 6:40 | UART 뮤텍스와 로그 태스크 | 문자열 보호 |
| 6:40 - 8:00 | 소프트웨어 타이머와 비상 점멸 타이머 | 500ms 점멸 사건 |
...
| 시간 | 내용 | 산출물 |
| --- | --- | --- |
| 0:00 - 1:10 | ADC 입력 안전과 원시값 확인 | PA0, PA1 범위 기록 |
| 1:10 - 2:40 | ADC 스캔과 원형 DMA | adc_values[0], adc_values[1] |
| 2:40 - 4:00 | RGB PWM | 가변저항 밝기 제어 |
| 4:00 - 5:10 | CdS 히스테리시스 | 깜빡임 없는 자동 점등 |
| 5:10 - 6:30 | 태스크와 큐로 BCM 연결 | 입력, 제어, 출력 구조 |
| 6:30 - 7:20 | 스택, 큐, 유실 횟수 | 진단 로그 |
| 7:20 - 8:00 | 통합 시험과 정리 | 시험표 완료 |
