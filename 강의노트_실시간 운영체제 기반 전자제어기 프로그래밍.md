...
RTOS가 CPU의 처리 성능을 높이는 것은 아니다. CPU는 어느 한 시점에 하나의 명령만 수행한다. RTOS는 대기 중인 작업이 CPU를 계속 점유하지 않도록 하여, 실행 가능한 다른 작업에 CPU를 배분한다.

| 용어 | 의미 | 보드에서 확인하는 예 |
| --- | --- | --- |
| 주기 | 같은 작업이 시작되는 목표 간격 | 입력 태스크가 50ms마다 센서 샘플 생성 |
| 마감 시각(Deadline) | 결과가 늦으면 의미가 없어지는 시각 | `SW1` 에지 뒤 LED 상태 변경 |
...
| --- | --- | --- |
| 하드 실시간 | 한 번의 지연도 시스템 실패가 될 수 있음 | 보호 동작, 안전 차단처럼 늦은 결과가 의미 없는 기능 |
| 결과 유효 기한형 실시간 | 늦게 끝난 결과는 사용하지 않음 | 오래된 센서 샘플보다 다음 최신 샘플이 나은 입력 처리 |
| 소프트 실시간 | 늦으면 품질은 떨어지지만 기능은 계속 가능 | UART 상태 메시지, 진단 로그 |

실습에서 LED와 UART는 시간 흐름을 관찰하기 위한 수단이다. 실제 제어 설계에서는 각 기능의 주기, 최대 허용 응답 시간, 최악 실행 시간을 먼저 정한 뒤 태스크와 우선순위를 결정한다.
...
`osKernelStart` 전에 큐, 세마포어, 뮤텍스, 소프트웨어 타이머와 태스크를 생성한다. `osKernelStart` 뒤에는 스케줄러가 태스크에 CPU 사용권을 배분하므로, 정상이라면 `main()`의 마지막 `while (1)`은 실행되지 않는다. RTOS 제어 코드를 이 `while` 문에 넣지 않는다. 태스크 또는 객체의 핸들은 객체 자체가 아니라 커널이 관리하는 객체를 가리키는 식별자(void*)이다. 생성 함수가 `NULL`을 반환하면 해당 핸들을 사용하지 않고 즉시 원인을 확인한다.
...
큐는 코드 상태에서 다음과 같이 직접 생성한다.

| 이름 | 갯수 | 아이템 크기 | 역할 |
| --- | --- | --- | --- |
| ledCtrl | 4 | uint32_t | LED 제어 메시지 |


`main.c`의 사용자 선언 영역에 다음을 추가한다. `ledCtrlHandle`는 `osKernelStart()`가 호출되기 전에 생성한다.
...
static osMessageQueueId_t ledCtrlHandle;
...
int main(void)
{
    ...
    /* add queues, ... */
    ledCtrlHandle = osMessageQueueNew (4, sizeof(LedCommand), NULL);
...
void StartButtonPollTask(void *argument)
{
    ...
    for (;;)
    {
        bool sw1_pressed = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_10) == GPIO_PIN_RESET;

        if (sw1_pressed && !previous_sw1_pressed)
        {
            LedCommand command = LED_COMMAND_TOGGLE;

            if (osMessageQueuePut(ledCtrlHandle, &command, 0U, 0U) != osOK)
            {
                ++queue_drop_count;
            }
        }

        previous_sw1_pressed = sw1_pressed;
        osDelay(10U);
    }
}
...
void StartLedOutputTask(void *argument)
{
    ...
    for (;;)
    {
        if (osMessageQueueGet(ledCtrlHandle, &command, NULL, osWaitForever) == osOK)
        {
            if (command == LED_COMMAND_TOGGLE)
            {
                HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5);
            }
        }
...
- `LedCommand`에 LED_COMMAND_TOGGLE 대신 LED_COMMAND_ON, LED_COMMAND_OFF를 추가한 후 SW1을 누르면 LED1을 켜고, SW2는 끄도록 개선해 본다.
- UART로부터 명령을 수신하는 uartRx 태스트를 추가한 후 큐를 이용해 `LED1`를 ON/OFF해 본다. 
  - PC에서 데이터를 전송할 때 Serial Monitor의 Line ending을 'LF'로 설정해야 한다. 
  - uart_retarget.h/uart_retarget.c를 추가한 후 `Core/Src/uart_retarget.c`를 CMakeList.txt의 `target_sources` 절에 추가 한다.
  - 반드시 printf 대신 `safe_printf`를 사용한다.

```sh
target_sources(${CMAKE_PROJECT_NAME} PRIVATE
    # Add user sources here
    Core/Src/uart_retarget.c
)
```
  ...
  for(;;)
  {
    for(;;)
    {
      if (fgets(buf, sizeof(buf), stdin) != NULL)
      {
          ...
          osMessageQueuePut(ledCtrlHandle, &command, 0U, 0U);
      }
    }
  }
}
...
5. DefaultTask를 hazard로 변경한다. 이진 세마포어는 코드 상태 직접 생성한다.

HAL이 생성한 `EXTI4_15_IRQHandler`는 `HAL_GPIO_EXTI_IRQHandler(GPIO_PIN_10)`를 거쳐 `HAL_GPIO_EXTI_Falling_Callback` 콜백까지 연결한다.

### 코드

```c
static osSemaphoreId_t sw1SemaHaldle;
static volatile uint32_t sw1_semaphore_full_count;

int main(void)
{
  /* add semaphores, ... */
  sw1SemaHaldle = osSemaphoreNew(1U, 0U, NULL);
...
void HAL_GPIO_EXTI_Falling_Callback(uint16_t gpio_pin)
{
    ...
    {
        if (osSemaphoreRelease(sw1SemaHaldle) != osOK)
        {
...
void StartHazardTask(void *argument)
{
    ...
    for (;;)
    {
        if (osSemaphoreAcquire(sw1SemaHaldle, osWaitForever) == osOK)
        {
...
코드에서 `pulseProducer`와 `pulseConsumer` 태스크를 추가한다. 두 태스크의 우선순위는 `Normal`, 스택 크기는 256 워드로 설정한다. 

```c
osThreadId_t pulseProducerHandle;
const osThreadAttr_t pulse_producer_attributes = {
  .name = "pulse_producer",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 256 * 4
};
osThreadId_t pulseConsumerHandle;
const osThreadAttr_t pulse_consumer_attributes = {
  .name = "pulse_consumer",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 256 * 4
};

static osSemaphoreId_t pulseSemaHandle;
...
int main(void)
{
  pulseSemaHandle = osSemaphoreNew(8U, 0U, NULL);

  pulseProducerHandle = osThreadNew(StartPulseProducerTask, NULL, &pulse_producer_attributes);
  pulseConsumerHandle = osThreadNew(StartPulseConsumerTask, NULL, &pulse_consumer_attributes);
}
...
void StartPulseProducerTask(void *argument)
{
    ...
    for (;;)
    {
        ...
        if (osSemaphoreRelease(pulseSemaHandle) != osOK)
        {
...
void StartPulseConsumerTask(void *argument)
{
    ...
    for (;;)
    {
        if (osSemaphoreAcquire(pulseSemaHandle, osWaitForever) == osOK)
        {
...
실행 중에는 디버거의 감시 창에서 `pulse_produced_count`, `pulse_consumed_count`, `pulse_overflow_count`, `osSemaphoreGetCount(pulse_semaphore)`를 관찰한다.
...
실습을 마친 뒤에는 `StartPulseProducerTask` 또는 `StartPulseConsumerTask`를 비활성화하거나 `StartPulseProducerTask` 태스크의 주기를 300 ms 이상으로 변경한다. 실제 인코더 펄스처럼 발생 시각이나 값도 함께 보관해야 하면 계수 세마포어 대신 큐로 구조체를 전달한다.

### 확인할 점

- `pulse_produced_count`가 `pulse_consumed_count`보다 빠르게 증가한다.
- 세마포어의 토큰 수가 8이면, `pulse_overflow_count`가 증가한다.
- `StartPulseProducerTask`의 주기를 300 ms로 바꾸면 오버플로가 사라지거나 크게 줄어든다.
- StartPulseConsumerTask를 10개로 늘려 테스트한다. 힙이 부족하면 새로운 태스트 생성에 실패하므로 총 힙 크기를 조절해 본다. 

```c
int main(void)
{
  for (int i = 0; i < 10; i++) {
    if (osThreadNew(StartPulseConsumerTask, (void*)i, &pulse_consumer_attributes) == NULL)
    {
      safe_printf("Task create failed: %d\n", i);
      break;
    }
  }
}
...
```c
osThreadAttr_t write_attributes = { //const를 사용하지 않음
  .name = "write",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 512 * 4
};

static osMutexId_t writeMutexHandle;
...
int main(void)
{
  ...
  for (int i = 0; i < 3; i++) 
  {
    osThreadNew(StartWriteTask, (void*)i, &write_attributes);
  }

}

static void WriteOnly(const char *text, uint16_t length)
{
  (void)HAL_UART_Transmit(&huart2, (uint8_t *)text, length, 20U);
}

static void WriteWithMutex(const char *text, uint16_t length)
{
    if (osMutexAcquire(writeMutexHandle, 20U) == osOK)
    {
        (void)HAL_UART_Transmit(&huart2,
                                (uint8_t *)text,
                                length,
                                20U);
        (void)osMutexRelease(writeMutexHandle);
    }
    else
    {
        ++log_drop_count;
...
void StartWriteTask(void *argument)
{
  ...
  const char* msg[]= {
    ...
    "it to say good-bye to the once you love",
    "gogogogogogogogogogogogogogogogogogogogogogogogo",
  };

  sprintf(buf, "Task %d: %s\n", id, msg[id]);
#if 1
  WriteOnly(buf, strlen(buf));
#else
  WriteWithMutex(buf, strlen(buf));
#endif
...
`osMutexNew`는 뮤택스 객체를 생성한다. `osMutexAcquire`는 뮤텍스를 획득하고, `osMutexRelease`는 반납한다. 3개의 `StartWriteTask` 태스크를 통해 `WriteOnly`는 뮤텍스 없이 시리얼로 데이터를 전송하고, `WriteWithMutex`는 뮤텍스와 함께 데이터를 전송한다.
...
- `WriteOnly`와 `WriteWithMutex`의 결과를 확인한다.
- 3개의 `StartWriteTask`의 우선 순위를 모두 다르게 설정한 후 `WriteOnly` 호출 결과를 확인한다.
```c
  for (int i = 0; i < 3; i++)
  {
    write_attributes.priority = osPriorityNormal + i;
    osThreadNew(StartWriteTask, (void*)i, &write_attributes);
  }
```
...
CubeMX에서 defaultTask 이름을 `hazardTimerTask`, 진입 함수는 `StartHazardTimerTask`로 변경한다. 우선순위는 `Normal`, 스택 크기는 256 워드로 설정한다.

```c
#define HAZARD_TICK_FLAG (1U << 0)

static osTimerId_t hazardTimerHandle;
...
int main(void)
{
  hazardTimerHandle = osTimerNew(HazardTimerCallback, osTimerPeriodic, NULL, NULL);
}
...
void StartHazardTimerTask(void *argument)
{
    ...
    if (osTimerStart(hazardTimerHandle, MillisecondsToKernelTicks(500U)) != osOK)
    {
        Error_Handler();
    }
    ...
    for (;;)
    {
        uint32_t flags = osThreadFlagsWait(HAZARD_TICK_FLAG, osFlagsWaitAny, osWaitForever);
...
CubeMX에서 defaultTask 이름을 `adcTask`, 진입 함수는 `StartAdcTask`로 변경한다. 우선순위는 `Normal`, 스택 크기는 256 워드로 설정한다.

```c
static volatile uint16_t adc_values[2];

static osTimerId_t adcTimerHandle;

static void AdcTimerCallback(void *argument)
{
    (void)argument;
...
int main(void)
{
    adcTimerHandle = osTimerNew(AdcTimerCallback, osTimerPeriodic, NULL, NULL);
}
...
void StartAdcTask(void *argument)
{
  ...
  HAL_ADC_Start_DMA(&hadc1, (uint32_t *)adc_values, 2U);

  osTimerStart(adcTimerHandle, MillisecondsToKernelTicks(100U));
...
- 소프트웨어 타이머어 대신 다음과 같이 센서 데이터를 큐에 넣으면, 이를 Uart로 출력하는 uartTask를 구현해 본다.
```c
typedef struct
{
    uint16_t dimmer;
...
void StartAdcTask(void *argument)
{
  sensorQueueHandle = osMessageQueueNew(4U, sizeof(SensorSample), NULL);
...
`PC7`/`TIM2_CH4`, `PB0`/`TIM1_CH2N`, `PA7`/`TIM14_CH1`의 PWM 설정과 밝기 변화 방향은 `임베디드 시스템 인터페이스 회로` 교재에서 확인한다. 이 실습에서는 `실습 10`을 재 사용하므로 중복 코드는 배제햐였다. RGB PWM을 스케줄러 시작 전에 `StartAdcTask`에서 한 번만 시작하고, 이후 비교 레지스터를 변경하는 코드는 `StartDimmerTask` 하나에만 둔다. `PB0`은 보완 출력이므로 `HAL_TIMEx_PWMN_Start(&htim1, TIM_CHANNEL_2)` 호출로 시작해야 한다.
...
void StartAdcTask(void *argument)
{
  ...
  for (;;)
  {
    SensorSample sample = (SensorSample){
      .dimmer = adc_values[0],
      .light = adc_values[1],
      .tick = osKernelGetTickCount()
    };
...
void StartDimmerTask(void *argument)
{
  (void)argument;
  SensorSample sample;
  ...
  for (;;)
  {
    ...
    {
        ApplyRgbBrightness(sample.dimmer);
    }
  }
}
```
...
### 코드 해설
`StartAdcTask`는 50ms 주기(osDelayUntil)로 ADC 변환값을 읽어 큐에 안전하게 전송하는 생산자(Producer) 역할을 수행하고, `StartDimmerTask`는 큐로부터 메시지를 수신(osWaitForever)할 때까지 대기하다가 데이터가 도착하면 즉시 PWM 비교값을 변경하는 소비자(Consumer) 역할을 담당한다.

ADC 데이터가 실시간으로 갱신되도록 태스크 시작 시점에 `HAL_ADC_Start_DMA` 함수를 추가하여 하드웨어 동작성을 확보한다.
...
void StartControlTask(void *argument)
{
    ...
    for (;;)
    {
        ...
        if (osMessageQueueGet(sensor_queue, &sample, NULL, osWaitForever) == osOK)
        {
            OutputCommand command;
            headlamp_enabled = UpdateHeadlampState(headlamp_enabled, sample.light);
            ...
            if (osMessageQueuePut(output_queue, &command, 0U, 0U) != osOK)
            {
...
void StartOutputTask(void *argument)
{
    ...
    for (;;)
    {
        ...
        if (osMessageQueueGet(output_queue, &command, NULL, osWaitForever) == osOK)
        {
            uint32_t sensor_to_output_ticks = osKernelGetTickCount() - command.source_tick;
...
- `StartAdcTask`가 최신 ADC 샘플을 `SensorSample`로 복사하여 `sensor_queue`에 전달한다.
